        ...
        {pluginType === "CPP" ? (
          <div className="grid gap-2 min-w-0 sm:grid-flow-col">
            <a
              ...
              className="btn btn-primary grid grid-flow-col place-items-center gap-2 text-[0.9375rem] px-5 py-2.5 bg-slate-800 dark:bg-slate-900 border border-slate-700 rounded-sm font-semibold text-white hover:bg-slate-700 dark:hover:bg-slate-800 transition-colors duration-150 no-underline min-w-0"
            >
              <Download size={16} /> Linux (.so)
            </a>
            <a
              href={`/api/v1/download/${slug}/${selectedVersion.version}?platform=windows`}
              className="btn btn-primary grid grid-flow-col place-items-center gap-2 text-[0.9375rem] px-5 py-2.5 rounded-sm font-semibold no-underline min-w-0"
            >
