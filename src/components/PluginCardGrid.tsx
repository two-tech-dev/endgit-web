...
function PluginBadge({ kind }: { kind: "pre" | "verified" | "featured" }) {
  ...
  const styles = {
    ...
    verified: "border-[#1890ff]/25 bg-[#1890ff]/15 text-[#1890ff]",
  };
  const labels = {
    pre: "Pre-release",
    verified: "Verified",
...
function CopyInstallButton({ slug }: { slug: string }) {
  ...
  return (
    <button
      type="button"
      onClick={copy}
      className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-sm border border-border bg-surface/90 text-text-muted opacity-0 transition-all hover:border-[#1890ff]/50 hover:bg-[#1890ff]/10 hover:text-[#1890ff] group-hover:opacity-100 group-focus-within:opacity-100"
...
function PluginCard({
  ...
  if (view === "list") {
    return (
      <article
        className="group relative grid gap-4 border border-border bg-surface-card p-4 transition-all hover:border-[#1890ff]/50 hover:bg-surface-secondary hover:shadow-[0_0_28px_rgba(24,144,255,0.18)] md:grid-cols-[auto_1fr_auto] md:items-center"
        style={{
  ...
  return (
    <article
      className="card group relative flex flex-col h-full overflow-hidden bg-surface-card p-0 transition-all hover:border-[#1890ff]/50 hover:bg-surface-secondary hover:shadow-[0_0_30px_rgba(24,144,255,0.18)]"
      style={{
  ...
  return (
    <div className="flex flex-col">
      ...
      <div
        className={
          view === "grid"
            ? "grid grid-cols-1 content-start items-stretch gap-3 sm:gap-4 lg:grid-cols-2 xl:grid-cols-3"
