...
export default function TypewriterHeading() {
  ...
  useEffect(() => {
    if (textIndex < fullText.length) {
      ...
      const timeout = setTimeout(() => setHighlightIndex(highlightIndex + 1), 70);
      return () => clearTimeout(timeout);
    } else {
  ...
  return (
    <div style={{ position: "relative", maxWidth: "720px", margin: "0 auto" }}>
      ...
      <h1 className="heading-1" style={{
        ...
        {fullText}<span style={{ color: "var(--accent-cyan)" }}>{highlightText}</span>
      </h1>
      ...
      <h1 className="heading-1" style={{
        ...
        <span style={{
          opacity: (textIndex < fullText.length || highlightIndex < highlightText.length) ? 1 : (showCursor ? 1 : 0),
