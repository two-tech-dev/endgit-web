...
function CodeBlock({ language, code }: { language: string; code: string }) {
  ...
  return (
    <div className="my-6 overflow-hidden rounded-md border border-[#7c3aed]/25 bg-[#111827] shadow-[0_0_24px_rgba(124,58,237,0.10)]">
      <div className="grid grid-cols-[1fr_auto] items-center border-b border-white/10 bg-white/[0.035] px-4 py-2">
        <span className="font-mono text-[11px] font-bold uppercase tracking-wider text-[#c4b5fd]">
          {displayLanguage}
        </span>
        <button
          type="button"
          onClick={copy}
          className="grid h-7 grid-cols-[auto_1fr] items-center gap-1.5 rounded-sm border border-white/10 px-2 text-[11px] text-slate-300 transition-colors hover:border-[#7c3aed]/50 hover:bg-[#7c3aed]/15 hover:text-white"
        >
          {copied ? (
      ...
      components={{
        ...
        code({ inline, className, children, ...props }: any) {
          ...
          return !inline && match ? (
            ...
            <code
              className="rounded-sm border border-[#7c3aed]/25 bg-[#7c3aed]/12 px-1.5 py-0.5 font-mono text-[0.88em] text-[#c4b5fd]"
              {...props}
            >
              {children}
            </code>
