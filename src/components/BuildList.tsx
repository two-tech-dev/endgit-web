...
function StatusIcon({ status }: { status: string }) {
  switch (status) {
    ...
    case "RUNNING":
      return <Loader2 size={16} className="animate-spin text-[#1890ff]" />;
...
export default function BuildsList({
  ...
  return (
    <>
      ...
      {!hideHeader && (
        <div className="mb-5">
          <div className="mb-2 flex flex-wrap items-center gap-3">
            ...
            <span className="rounded-sm border border-[#1890ff]/25 bg-[#1890ff]/10 px-2.5 py-0.5 text-xs font-semibold text-[#1890ff] ">
              Today ({today})
            </span>
          </div>
      ...
      {/* Search */}
      <div className="mb-4">
        <div className="card flex items-center gap-3 rounded-sm px-3 py-0 transition-colors focus-within:border-[#1890ff]/45 lg:px-4">
          ...
          <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-2 xl:grid-cols-3">
            {filteredBuilds.map((build: any, i: number) => (
              <Link
                ...
                className={`card flex flex-col overflow-hidden border-l-4 bg-surface-card p-0 no-underline transition-all hover:border-[#1890ff]/45 hover:bg-surface-secondary hover:shadow-[0_0_24px_rgba(24,144,255,0.14)] ${statusBorderClass(build.status)}`}
                style={{
