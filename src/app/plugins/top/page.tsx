...
export default async function TopPluginsPage() {
  const { data: responseData } = await fetchApi("/api/v1/plugins?sort=downloads&order=desc&pageSize=50");
  const plugins = responseData?.data?.plugins || [];

  return (
    <div className="container" style={{ padding: "var(--space-8) 0" }}>
      <div style={{ marginBottom: "var(--space-8)" }}>
        <Link href="/plugins" style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "var(--text-muted)", marginBottom: "var(--space-4)", fontSize: "0.875rem" }}>
        ...
        {plugins.map((plugin: any, index: number) => {
          ...
          return (
            <a href={`/plugins/${plugin.slug}`} key={plugin.id} className="card" style={{
              ...
              <div style={{ padding: "var(--space-4)", display: "flex", gap: "var(--space-4)", flexWrap: "wrap" }}>
                ...
                <div style={{ flex: 1, minWidth: "200px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)", marginBottom: "4px" }}>
                    <h3 style={{ margin: 0, fontSize: "1.125rem", color: "var(--text-primary)" }}>{plugin.displayName}</h3>
                  </div>
              ...
              <div style={{
                marginTop: "auto", borderTop: "1px solid var(--border-color)",
