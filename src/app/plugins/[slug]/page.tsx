...
export default async function PluginDetailPage({
 ...
 <div className="w-[88px] h-[88px] lg:w-[100px] lg:h-[100px] shrink-0 rounded-2xl overflow-hidden bg-surface-secondary flex items-center justify-center border border-border/30 shadow-xs">
 <PluginImage
 ...
 <h1 className="text-3xl lg:text-[2rem] font-bold text-text-primary m-0 tracking-tight">
 {plugin.displayName}
 </h1>
 {VERIFIED_ORGS.includes(repoOwnerDetail || "") && (
 <span title="This plugin is officially supported by EndstoneMC/EndGit" className="text-[#1890ff]">
 <BadgeCheck size={24} />
 </span>
 )}
 ...
 <p className="text-text-secondary mt-1.5 text-base lg:text-[1.0625rem] leading-relaxed max-w-3xl truncate">
 {plugin.description}
 </p>
 
 <div className="flex items-center gap-4 mt-3 text-sm font-medium text-text-muted">
 <span className="flex items-center gap-1.5 text-text-secondary" title="Total Downloads">
 <Download size={18} />
 {(plugin.downloads || 0).toLocaleString()}
 </span>
 <span className="flex items-center gap-1.5 text-text-secondary" title="Heat Score">
 <Flame size={18} />
 ...
 {plugin.pluginType === "PYTHON" ? "Python" : plugin.pluginType === "CPP" ? "C++" : plugin.pluginType}
 </span>
 )}
 </div>
 </div>
