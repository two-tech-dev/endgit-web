...
export default async function SubmitReviewPage({ params }: { params: { id: string } }) {
  ...
  if (!build) {
    return (
      <div className="container" style={{ paddingTop: "var(--space-10)", textAlign: "center" }}>
        ...
        <p className="text-muted">The build you are looking for does not exist.</p>
      </div>
    );
  }
  ...
  if (build.status !== "SUCCESS") {
    return (
      <div className="container" style={{ paddingTop: "var(--space-10)", textAlign: "center" }}>
        ...
        <p className="text-muted">Only successful builds can be submitted for review.</p>
      </div>
    );
  }
  ...
  return (
    <div className="container" style={{ paddingTop: "var(--space-10)", paddingBottom: "var(--space-16)" }}>
      <SubmitReviewForm buildId={build.id} buildNumber={build.buildNumber} plugin={build.plugin} />
    </div>
  );
}
